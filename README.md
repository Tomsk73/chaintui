...
**CVEs:** press `v` on a repo for the CVE list of its `latest` image, or `v` on a tag for that exact image (resolved by digest). Rows are one CVE per affected package, worst severity first, with CVSS, the installed version, and the fixed version when one exists; the footer summarises the scan (`5 CVEs │ high 5 │ grype 0.116.1 │ scanned 21h ago`). `f` narrows to CVEs with a known fix, `d` shows the full record (PURL, location, NVD link), `/` filters locally (a severity or a package name both work), and `s` saves the list as CSV. The vuln report list API doesn't include the CVEs themselves, so chaintui reads the newest scan report for the image and parses the scanner's raw output (grype, or trivy if that produced the report), downloading it from the signed URL when the report is too large to inline.
